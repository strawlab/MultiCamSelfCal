...
# $Author: svoboda $
# $Revision: 2.0 $
# $Id: im2pmultiproc.pl,v 2.0 2003/06/19 12:07:11 svoboda Exp $
# $State: Exp $
...
foreach $process (@processes) {
	...
	$process->{'mcmd'} = "ssh $process->{'compname'} /pub/bin/matlab -nosplash -nojvm < $process->{'scriptName'} > $process->{'scriptName'}.out &";
}
